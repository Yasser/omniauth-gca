...
class GcaSsoApi
  ...
  def get
    if @user_token
      @response = @client[@request_uri].get :Authorization => "Basic #{@user_token}", grant_type: 'client_credentials'
    else
  ...
  def post
    if @user_token
      @response = @client[@request_uri].post :params => @params, :Authorization => "Basic #{@user_token}", grant_type: 'client_credentials'
    else
